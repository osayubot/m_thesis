...
def main(
    ...
    Args:
        ...
        use_reference_based: 基準進行ベースのUMAPを使用するか
        reference_progressions: 基準進行の辞書（Noneの場合はデフォルトを使用）
        n_neighbors: UMAPの近傍数
        min_dist: UMAPの最小距離
    """
    ...
    if export_json:
        ...
        if use_reference_based:
            ...
            for ref_name, ref_prog in reference_progressions.items():
                print(f"\n{'='*60}")
                print(f"Processing reference progression (UMAP): {ref_name}")
                ...
                # JSONファイル名を決定
                filename = filename_map.get(ref_name, f"umap_{ref_name}_pie_data.json")
